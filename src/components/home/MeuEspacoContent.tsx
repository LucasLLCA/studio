...
import { Loader2, Trash2, MoreHorizontal, Share2, Pencil, ChevronDown, Bell } from 'lucide-react';
  ...
  return (
    <>
        ...
        <div className="mb-4">
          <div className="flex items-center justify-between mb-2">
            <h3 className="text-sm font-semibold text-foreground flex items-center gap-1.5">
              ...
              {unreadFeedCount > 0 && (
                <Badge variant="destructive" className="text-2xs px-1.5 py-0 ml-1">
                  {unreadFeedCount}
                </Badge>
              )}
            </h3>
            ...
            {recentFeedItems.map((entry) => {
              ...
              return (
                <div
                  key={entry.id}
                  className={cn(
                    "flex items-start gap-2 rounded px-2 py-1.5 transition-colors hover:bg-muted/30 border-l-2 cursor-pointer",
                    isUnread ? "border-l-primary bg-info-light/30" : "border-l-transparent"
                  )}
      ...
      <TabListWrapper isLoading={isLoading} isEmpty={tags.length === 0} emptyMessage="Nenhum processo salvo ainda.">
        <div className="space-y-2">
          {tags.map((tag) => {
            ...
            return (
              <div key={tag.id} className="rounded-md border border-border transition-colors">
                {/* Tag header */}
                <button
                  onClick={() => toggleExpand(tag.id)}
                  className="w-full text-left p-3 hover:bg-muted/30 transition-colors flex items-center gap-2"
                >
                  {tag.cor && (
                    <span className="w-3 h-3 rounded-full shrink-0" style={{ backgroundColor: tag.cor }} />
                  )}
                  <p className="font-medium text-foreground flex-1">{tag.nome}</p>
                  <span className="text-xs text-muted-foreground">{tag.total_processos} {tag.total_processos === 1 ? 'processo' : 'processos'}</span>
                  <DropdownMenu>
                  ...
                  <ChevronDown className={`h-4 w-4 text-muted-foreground/70 transition-transform ${isExpanded ? 'rotate-180' : ''}`} />
                </button>
      ...
      <DeleteConfirmDialog
        ...
        title="Excluir grupo"
        description="Esta acao ira excluir o grupo, todos os processos salvos nele e seus compartilhamentos. Esta acao nao pode ser desfeita."
